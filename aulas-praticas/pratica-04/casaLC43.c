...
char* multiply( char* num1, char* num2 ) {
    ...
    // se qualquer uma das entradas for 0, o resultado vai ser "0"
    ...
    // inicializacao da string resultante ( ja pensando nas futuras somas )
	...
	for ( i = sizeNum1 - 1; i >= 0; i-- ) {
		int carry = 0;

		for ( j = sizeNum2 - 1; j >= 0; j-- ) {
			int mult = ( num1[i] - '0' ) * ( num2[j] - '0' );
			int total = ( mult + carry ) + ( res[pos] - '0' );
		...
		res[pos] = carry + res[pos]; // carry final de cada linha da multiplicacao

        // quando terminar uma linha, a proxima volta uma casa anterior ao ultimo comeco
		if ( i != 0 ) {
			pos = ( i + sizeNum2 ) - 1; // = maxSize variando, - 1 do \0
		}
	}
	...
	if ( res[0] == '0' ) {
		...
		res = ( char * ) realloc( res, sizeof( char ) * maxSize );
	}
